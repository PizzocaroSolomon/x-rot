...
bool LslidarC32Decoder::loadParameters() {
    ...
    pnh.param<string>("fixed_frame_id", fixed_frame_id, "world");
    pnh.param<string>("child_frame_id", child_frame_id, "LSC_32");
...
bool LslidarC32Decoder::initialize() {
    ...
    for (size_t scan_idx = 0; scan_idx < 32; ++scan_idx) {
        ...
        sweep_data->scans[scan_idx].altitude = scan_altitude[remapped_scan_idx];
        // std::cout << "remapped altitude : " << scan_idx << "  " << round(sweep_data->scans[scan_idx].altitude*180/3.1415) << std::endl;
    }
...
//                new pcl::PointCloud<pcl::PointXYZI>());
    ...
    for (size_t i = 0; i < 32; ++i) {
        ...
        //point_cloud->header.stamp = sweep_fpga_time.toNSec();

        if (scan.points.size() == 0) continue;
        size_t j;
        VPoint point;
        for (j = 1; j < scan.points.size()-1; ++j) {
			point.timestamp = sweep_fpga_time.toSec() - (scan.points.size()-1 - j)*0.00005;  // time interval for each point is 50us
    ...
    point_cloud->header.stamp = ros::Time::now().toNSec() / 1000; // the pointcloud header stamp is in milliseconds
    ...
    for (size_t blk_idx = 0; blk_idx < BLOCKS_PER_PACKET; ++blk_idx) {
        ...
        for (size_t blk_fir_idx = 0; blk_fir_idx < FIRINGS_PER_BLOCK; ++blk_fir_idx){
            ...
            for (size_t scan_fir_idx = 0; scan_fir_idx < SCANS_PER_FIRING; ++scan_fir_idx){
                ...
                double A2 = 4*DEG_TO_RAD;
                double A1 = 0*DEG_TO_RAD;
                double A_vec[32] = {A2,A2,A1,A1,A2,A2,A1,A1,A2,A2,A1,A1,A2,A2,A1,A1,A2,A2,A1,A1,A2,A2,A1,A1,A2,A2,A1,A1,A2,A2,A1,A1};

                firings[fir_idx].azimuth[scan_fir_idx] = firings[fir_idx].firing_azimuth + A_vec[scan_fir_idx];

                // if(scan_fir_idx%2==0)
                // {
                // //if(abs(firings[fir_idx].firing_azimuth-360*DEG_TO_RAD)>4*DEG_TO_RAD)
                //     firings[fir_idx].azimuth[scan_fir_idx] = firings[fir_idx].firing_azimuth+10*DEG_TO_RAD;
                // }
                // else
                // {
                //     firings[fir_idx].azimuth[scan_fir_idx] = firings[fir_idx].firing_azimuth;
                ...
                firings[fir_idx].distance[scan_fir_idx] = static_cast<double>(raw_distance.distance) * DISTANCE_RESOLUTION;
               
                // Intensity
                firings[fir_idx].intensity[scan_fir_idx] = static_cast<double>(raw_block.data[byte_idx+2]);
            }
        }
    }
...
void LslidarC32Decoder::packetCallback(
    ...
    sweep_rec_time = msg->rec_stamp;
    sweep_fpga_time = msg->fpga_stamp;
    ...
    if (is_first_sweep &&
        ...
        if (is_first_sweep) {
            ...
            sweep_start_time = msg->fpga_stamp.toSec() +
    ...
    for (size_t fir_idx = start_fir_idx; fir_idx < end_fir_idx; ++fir_idx) {
        for (size_t scan_idx = 0; scan_idx < SCANS_PER_FIRING; ++scan_idx) {
            ...
            // double x = firings[fir_idx].distance[scan_idx] * cos_scan_altitude[scan_idx] * cos(firings[fir_idx].azimuth[scan_idx]);
            // double y = firings[fir_idx].distance[scan_idx] * cos_scan_altitude[scan_idx] * sin(firings[fir_idx].azimuth[scan_idx]);
            // double z = firings[fir_idx].distance[scan_idx] * sin_scan_altitude[scan_idx];

            double x = firings[fir_idx].distance[scan_idx] * cos_scan_altitude[scan_idx] * sin(firings[fir_idx].azimuth[scan_idx]);
            double y = firings[fir_idx].distance[scan_idx] * cos_scan_altitude[scan_idx] * cos(firings[fir_idx].azimuth[scan_idx]);
            double z = firings[fir_idx].distance[scan_idx] * sin_scan_altitude[scan_idx];

            // double x = firings[fir_idx].distance[scan_idx] * cos_scan_altitude[scan_idx] * sin_azimuth;
            // double y = firings[fir_idx].distance[scan_idx] * cos_scan_altitude[scan_idx] * cos_azimuth;
            // double z = firings[fir_idx].distance[scan_idx] * sin_scan_altitude[scan_idx];
